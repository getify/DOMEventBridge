...
// shim for Array.isArray() in non-ES5
...
(function (name, context, definition) {
  if (typeof module != 'undefined' && module.exports) module.exports = definition();
...
})('DOMBridge', this, function (name, context) {
	...
	function makeDOMCaptureEventHandler(sel,elems,evtName) {
		var tmp, matchSelector = [], i;

		// return the cached handler if any
		if (dom_capture_handlers[sel] &&
			dom_capture_handlers[sel][elems] &&
			dom_capture_handlers[sel][elems][evtName]
		) {
			return dom_capture_handlers[sel][elems][evtName];
		}

		// construct a matching selector test for the event target in question
		tmp = elems.split(/\s*,\s*/);
		for (i=0; i<tmp.length; i++) {
			if (tmp[i]) {
				matchSelector.push(sel + " " + tmp[i]);
			}
		}
		matchSelector = matchSelector.join(",") || sel;

		// make sure cache structure is ready
		dom_capture_handlers[sel] = dom_capture_handlers[sel] || {};
		dom_capture_handlers[sel][elems] = dom_capture_handlers[sel][elems] || {};

		// cache handler
		dom_capture_handlers[sel][elems][evtName] = function(evt) {
			var $el = dom.$(evt.target), elementType = $el.attr("rel"), elementTypes;

			// is event target one we are delegating for?
			if (selects.is(evt.target,matchSelector)) {
	...
	// DOM bubble event listener
	function DOMBubbleEventHandler(evt) {
		var $el = dom.$(evt.target), elementType = $el.attr("rel"), elementTypes;
	...
	// bridge events from DOM element(s) using event delegation
	// NOTE: for `elems` param, if you don't pass it at all or if it's falsy, it defaults to the `defaultTargets` setting
	//       otherwise, it's assumed to be the delegated elements selector for event delegation listening
	//       the same goes for the `evts` param: if falsy, will default to `defaultEvents` setting.
	function bridgeFrom(sel,elems,evts) {
		var capturingEvents = [], tmp, i, j;

		if (!sel) sel = "body";
		if (!elems) elems = publicAPI.defaultTargets.join(", ");
		else if (Array.isArray(elems)) elems = elems.join(", ");
		if (evts && Array.isArray(evts)) evts = evts.join(" ");
		if (!evts || !evts.match(/[^\s]/)) evts = publicAPI.defaultEvents.join(" ");
		if (capturingSupported) {
			if (~evts.indexOf("focus")) capturingEvents.push("focus");
			if (~evts.indexOf("blur")) capturingEvents.push("blur");
			if (~evts.indexOf("change")) capturingEvents.push("change");
		}
		...
		if (!(sel in bridges) ||
			...
			tmp = selects.find(sel);
			for (i=0; i<tmp.length; i++) {
				events.handle(tmp[i],evts,elems,DOMBubbleEventHandler);

				if (capturingEvents.length) {
					for (j=0; j<capturingEvents.length; j++) {
						tmp[i].addEventListener(
							/*event=*/capturingEvents[j],
							/*handler=*/makeDOMCaptureEventHandler(sel,elems,capturingEvents[j]),
							/*capturing=*/true
						);
					}
				}
	...
	//       otherwise, it's assumed to be the delegated elements selector for event delegation listening
	//       the same goes for the `evts` param: if falsy, will default to `defaultEvents` setting.
	function removeBridge(sel,elems,evts) {
		var capturingEvents = [], tmp, i, j;
		...
		if (bridges[sel] && bridges[sel][evts] && bridges[sel][evts][elems]) {
			tmp = selects.find(sel);
			for (i=0; i<tmp.length; i++) {
				events.remove(tmp[i],evts,elems,DOMBubbleEventHandler);
				delete bridges[sel][evts][elems];

				if (capturingEvents.length) {
					for (j=0; j<capturingEvents.length; j++) {
						tmp[i].removeEventListener(
							/*event=*/capturingEvents[j],
							/*handler=*/makeDOMCaptureEventHandler(sel,elems,capturingEvents[j]),
							/*capturing=*/true
						);
	...
	// * inSelector: string; a DOM CSS selector which must be a collection that contains the event's target
	function filter(opts) {
		...
		function checkFilter(el,elementType,evt) {
			...
			// `inSelector` filter specified but not matched?
			if (ok && "inSelector" in opts && !selects.is(el,opts.inSelector + "," + opts.inSelector + " *")) {
				ok = false;
			}
		...
		// factory for filter event handlers
		function makeHandler(evtName) {
			return function(el,elementType,evt){
				// handle event?
		...
		opts = opts || {};
		if (opts.elementTypes && typeof opts.elementTypes === "string") {
			opts.elementTypes = opts.elementTypes.split(/\s+/);
		}
		if (opts.eventTypes && typeof opts.eventTypes === "string") {
		...
		for (i=0; i<bridge_event_types.length; i++) {
			// make filter handler and store it (for unbinding)
			filter[bridge_event_types[i]] = makeHandler(bridge_event_types[i].replace(/^.*?:/,""));

			// set up listening binding for filter handler
			listener.on(bridge_event_types[i],filter[bridge_event_types[i]]);
		}

		filter.makeHandler = makeHandler;
	...
	function registerBridgeEventType(bridgeEvtType) {
		if (bridgeEvtType === "listener") throw new Error("DOMBridge: Illegal bridge event type");
		if (!~bridge_event_types.indexOf(bridgeEvtType)) {
			...
			// update all existing filters with this new bridge event type
			for (var i=0; i<filters.length; i++) {
				filters[i][bridgeEvtType] = filters[i][bridgeEvtType] || filters[i].makeHandler(bridgeEvtType.replace(/^.*?:/,""));
				filters[i].listener.on(bridgeEvtType,filters[i][bridgeEvtType]);
			}
		}
	}
	...
	var bridge_event_handlers = {}, i,
		...
		dom_capture_handlers = {},

		publicAPI,
		capturingSupported = !!document.addEventListener
	;
	...
	publicAPI = global.EventHub.DOMBridge = {
		defaultTargets: [ "a", "button", "input[type='button']" ],
		defaultEvents: [ "click", "mouseover", "mouseout", "mousedown", "mouseup", "keypress", "focus", "blur", "change" ],
		events: filter(/*empty*/), // set up a default unfiltered listener
		bridgeFrom: bridgeFrom,
		removeBridge: removeBridge,
		stopBridging: stopBridging,
		resumeBridging: resumeBridging,
		filter: filter,
	...
	return global.EventHub.DOMBridge;
});